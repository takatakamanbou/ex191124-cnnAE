...
class AE(nn.Module):
    ...
    def __init__(self, in_shape, C1, C2, H):
        ...
        self.conv2_shape = np.array(X.shape[1:])
        print('# conv2_shape:', self.conv2_shape)

        X = X.view((-1, np.prod(self.conv2_shape)))
        ...
        X = X.view((-1,) + tuple(self.conv2_shape))
        print('# reshaped output:', X.shape)

        self.tconv1 = nn.Sequential(
            nn.ConvTranspose2d(self.conv2_shape[0], C1, kernel_size=4, stride=2, padding=1),
            nn.LeakyReLU()
        )
        X = self.tconv1(X)
        ...
        self.tconv2 = nn.Sequential(
            nn.ConvTranspose2d(self.tconv1[0].out_channels, in_shape[0], kernel_size=4, stride=2, padding=1, bias=False)
        )
        X = self.tconv2(X)
